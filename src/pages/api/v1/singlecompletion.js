...
const postUrl = async (req, res) => {
  ...
  if (!messages || !Array.isArray(messages)) {
    return res.status(400).json({ error: 'Invalid or missing "messages" field' });
  }
  if (!model || typeof model !== 'string') {
    return res.status(400).json({ error: 'Invalid or missing "model" field' });
  }
  if (temperature === undefined || typeof temperature !== 'number') {
    return res.status(400).json({ error: 'Invalid or missing "temperature" field' });
  }
  ...
  try {
    ...
    if (model.includes('gpt')) {
      ...
      provider = 'anthropic';
      completion = await createClaudeCompletionMessage(messages, model, temperature);
    } else {
      provider = 'ppxty';
      completion = await createPpxtyCompletionMessage(messages, model, temperature);
    }
    ...
    res.status(500).json({ error: 'Failed to generate completion. Please try again later.' });
  }
};
