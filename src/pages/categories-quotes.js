...
import {
  categories,
  promptTemplateCategoriesQuotes,
} from '../../utils/handleprompts/internal'
...
const CategoryQuotes = () => {
  ...
  const generateQuote = async (text, categories) => {
    const prompt = promptTemplateCategoriesQuotes(text, categories)
  ...
  const handleSubmit = async () => {
    ...
    const promises = urlsArray.map(async (url) => {
      const pageData = await handlePageParse(url)
      if (pageData) {
        const quote = await generateQuote(pageData.content, categories)
        return { ...pageData, quote }
      }
      return null
    })
